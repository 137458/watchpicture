...
- **严禁全量解压落盘**：压缩包一律内存级流式读取（Zip4j 引擎 + Coil 3 自定义 `Fetcher` 管道），大体积压缩包即开即显。豁免：图片解码 / 视频播放等无法流式消费的场景，允许单条目按需落盘到受容量预算约束的缓存目录（如 `cacheDir/playback`），仍严禁整包解压落盘。
...
| 何时读 | 文档 |
|---|---|
| 调研文档（加密归档加载、图片加载性能、解压引擎提速） | `docs/research/` |
| 更新日志 | `CHANGELOG.md` |
