...
class ArchiveExtractionCoordinatorTest {
    ...
    @Test
    fun `extractThumbnailDirect generates thumbnail without dumping full-res image to archiveDiskCache`() = runBlocking {
        val thumbDir = tempFolder.newFolder("thumb_cache")
        val thumbCache = ThumbnailDiskCache(thumbDir)
        val coordinator = ArchiveExtractionCoordinator(zipManager, archiveDiskCache)

        val thumbFile = coordinator.extractThumbnailDirect(
            file = solid7z,
        ...
        assertTrue("Thumbnail file must have non-zero length", thumbFile.length() > 0)

        // Crucial invariant: archiveDiskCache must NOT contain the full-resolution file!
        val fullCached = archiveDiskCache.get(solid7z, "01_page.jpg", password = null)
        assertNull("7z thumbnail extraction must NOT dump full-res image to archiveDiskCache", fullCached)
    }

    @Test
    fun `startBatchThumbnailSweep generates thumbnails for all entries in a single pass`() = runBlocking {
        val thumbDir = tempFolder.newFolder("thumb_batch_cache")
        val thumbCache = ThumbnailDiskCache(thumbDir)
        val coordinator = ArchiveExtractionCoordinator(zipManager, archiveDiskCache)

        val entries = listOf("00_page.jpg", "01_page.jpg", "02_page.jpg")
        ...
        for (e in entries) {
            val thumb = thumbCache.get(solid7z, e, 100, password = null)
            assertNotNull("Thumbnail for $e must be generated by batch sweep", thumb)
