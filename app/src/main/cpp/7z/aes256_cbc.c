...
static void aes256_decrypt_block(const aes256_cbc_ctx *ctx, const uint8_t in[16], uint8_t out[16]) {
    ...
    const uint32_t *rk = &ctx->round_keys[56];
    for (int c = 0; c < 4; c++) {
        uint32_t k = rk[c];
        state[0][c] ^= (uint8_t)(k >> 24);
        state[1][c] ^= (uint8_t)(k >> 16);
        state[2][c] ^= (uint8_t)(k >> 8);
        state[3][c] ^= (uint8_t)k;
    }
    ...
    for (int round = 13; round >= 1; round--) {
        ...
        for (int c = 0; c < 4; c++) {
            ...
            state[0][c] = Mul14(a0) ^ Mul11(a1) ^ Mul13(a2) ^ Mul9(a3);
            state[1][c] = Mul9(a0) ^ Mul14(a1) ^ Mul11(a2) ^ Mul13(a3);
            state[2][c] = Mul13(a0) ^ Mul9(a1) ^ Mul14(a2) ^ Mul11(a3);
            state[3][c] = Mul11(a0) ^ Mul13(a1) ^ Mul9(a2) ^ Mul14(a3);
        }
    }
...
void aes256_cbc_decrypt(aes256_cbc_ctx *ctx, const uint8_t *src, uint8_t *dst, size_t len) {
    ...
    for (size_t b = 0; b < blocks; b++) {
        ...
        uint8_t next_iv[16];
        memcpy(next_iv, ct, 16);

        aes256_decrypt_block(ctx, ct, pt);
