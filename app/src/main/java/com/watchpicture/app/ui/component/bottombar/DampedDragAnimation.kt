...
/**
 * 计算非对称速度拉伸（Velocity Stretch）：
 * 水平滑动时指示器沿滑动方向拉伸，垂直方向相应压缩，保持流体体积感。
 */
fun calculateVelocityStretch(velocity: Float): Pair<Float, Float> {
    val v = velocity / 10f
    val factorX = (v * 0.75f).coerceIn(-0.2f, 0.2f)
    val factorY = (v * 0.25f).coerceIn(-0.2f, 0.2f)
    val denomX = 1f - factorX
    val stretchX = if (denomX != 0f) 1f / denomX else 1f
    val compressY = 1f - factorY
    return Pair(stretchX, compressY)
}

/**
 * 计算底栏外壳橡皮筋面板拉伸阻尼位移（Rubber Band Panel Offset）。
 */
fun calculateRubberBandOffset(
    dragOffset: Float,
    totalWidth: Float,
    rubberBandMaxPx: Float,
): Float {
    if (totalWidth <= 0f) return 0f
    val fraction = (dragOffset / totalWidth).fastCoerceIn(-1f, 1f)
    return rubberBandMaxPx * fraction.sign * EaseOut.transform(abs(fraction))
}
